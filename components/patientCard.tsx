          ...
          >
            ...
            <div className="h-full w-1/6 border-[#F4F4F4] border-l-4 border-solid flex justify-between items-center flex-col">
              <div className="h-full w-full flex flex-col border-[#F4F4F4] border-b-4 text-xs items-center justify-center  cursor-pointer">
                <img src="/icons/chart-order.svg" alt="order" width={20} className="pointer-events-none select-none"/>
                Orders
                <span
              ...
              >
                <img src="/icons/chart-prn.svg" alt="prn" width={20} className="pointer-events-none select-none"/>
                PRN
              ...
              <div className="h-full w-full flex flex-row border-[#F4F4F4] text-xs items-center gap-1 justify-center  cursor-pointer">
                <div>
                  <img src="/icons/chart-status.svg" alt="status" width={15} 
                  className="pointer-events-none select-none"/>A
                </div>
                <div>
                  <img src="/icons/chart-status.svg" alt="status" width={15} 
                  className="pointer-events-none select-none"/>S
                </div>
                <div>
                  <img src="/icons/chart-status.svg" alt="status" width={15} className="pointer-events-none select-none"/>D
                </div>
              </div>
            </div>

            <div className="h-full w-1/6 border-[#F4F4F4] border-l-2 border-solid flex justify-center items-center flex-col bg-[#007C851A]">
              {patientWithMedicationLogsToday[0]?.medicationlogs.length > 0 &&
              patientWithMedicationLogsToday[0]?.medicationlogs[0]
                ?.medicationLogStatus === "pending" ? (
                <div className="bg-[#FACC15] rounded-full p-1.5">
                  <img src="icons/card-list.svg" alt="list" width={20} className="pointer-events-none select-none" />
                  ...
                  {patient.medicationlogs.filter(
                    ...
                    <span className="absolute h-4 w-4  text-xs font-light rounded-full bg-red-600 text-white pointer-events-none select-none">
                      {
                        patient.medicationlogs.filter(
                          (log: { medicationLogStatus: string; medicationLogsTime: any }) =>
                            log.medicationLogStatus === "pending" && log.medicationLogsTime <= currentTime
                        ).length
                      }
                    </span>
                  )}
